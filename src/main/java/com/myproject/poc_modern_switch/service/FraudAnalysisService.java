...
import com.myproject.poc_modern_switch.normalizer.ProviderValueNormalizer;
...
public class FraudAnalysisService {
    ...
    private final ProviderValueNormalizer normalizer = new ProviderValueNormalizer();

    public FraudAnalysis analyze(String cpf) {

        Object bureauValue = bureauProvider.analyze(cpf);

        Object deviceValue = deviceProvider.analyze(cpf);

        Object geoValue = geoProvider.analyze(cpf);

        Object behavioralValue = behavioralProvider.analyze(cpf);

        int bureauScore = normalizer.normalize(bureauValue);

        int deviceRisk = normalizer.normalize(deviceValue);

        int geoRisk = normalizer.normalize(geoValue);

        int behavioralRisk = normalizer.normalize(behavioralValue);
